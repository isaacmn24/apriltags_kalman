...
class SpawnAndMoveModel:
    ...
    # 45-degree rotation around the z-axis to face the x-axis
    angle = 0 * (3.14159 / 180)  # Convert 45 degrees to radians
    ...
    def spawn_model(self):
        try:
            ...
            initial_pose = Pose(Point(SpawnAndMoveModel.initial_x, SpawnAndMoveModel.initial_y, SpawnAndMoveModel.initial_z), Quaternion(SpawnAndMoveModel.orientation_x, SpawnAndMoveModel.orientation_y, SpawnAndMoveModel.orientation_z, SpawnAndMoveModel.orientation_w))  
    ...
    def move_model(self):
        ...
        while not rospy.is_shutdown():
            ...
            try:
                ...
                if SpawnAndMoveModel.initial_x + x  > SpawnAndMoveModel.target_x:
                    forward = False

                if SpawnAndMoveModel.initial_x + x  < -SpawnAndMoveModel.target_x:
                    forward = True

                model_state.pose.position.x = SpawnAndMoveModel.initial_x + x  # Change here for x-axis movement
                model_state.pose.position.y = SpawnAndMoveModel.initial_y
                model_state.pose.position.z = SpawnAndMoveModel.initial_z

            
                model_state.pose.orientation.x = SpawnAndMoveModel.orientation_x
                model_state.pose.orientation.y = SpawnAndMoveModel.orientation_y
                model_state.pose.orientation.z = SpawnAndMoveModel.orientation_z
                model_state.pose.orientation.w = SpawnAndMoveModel.orientation_w
